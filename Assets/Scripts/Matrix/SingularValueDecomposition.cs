...
namespace DotNetMatrix
{
	...
	public class SingularValueDecomposition : System.Runtime.Serialization.ISerializable
	{
		...
		public SingularValueDecomposition(GeneralMatrix Arg)
		{
			...
			// Initialize.
			float[][] A = Arg.ArrayCopy;
			m = Arg.RowDimension;
			n = Arg.ColumnDimension;
			int nu = System.Math.Min(m, n);
			s = new float[System.Math.Min(m + 1, n)];
			U = new float[m][];
			for (int i = 0; i < m; i++)
			{
				U[i] = new float[nu];
			...
			while (p > 0)
			{
				...
				else
				{
					...
					for (ks = p - 1; ks >= k; ks--)
					{
						...
						float t = (ks != p?System.Math.Abs(e[ks]):0.0f) + (ks != k + 1?System.Math.Abs(e[ks - 1]):0.0f);
				...
				switch (kase)
				{
					...
					case 3:  
					{
						...
						float scale = System.Math.Max(System.Math.Max(System.Math.Max(System.Math.Max(System.Math.Abs(s[p - 1]), System.Math.Abs(s[p - 2])), System.Math.Abs(e[p - 2])), System.Math.Abs(s[k])), System.Math.Abs(e[k]));
		...
		/// </returns>
		virtual public float[] SingularValues
		{
			get
			{
				return s;
		...
		{
			float eps = (float)System.Math.Pow(2.0f, - 52.0f);
